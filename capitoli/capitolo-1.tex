...
Tale framework definisce i ruoli e il lavoro all'interno del team, denominato quindi Team Scrum.\\
...
\begin{itemize}
\item \textit{Product Owner}: può essere definito come il portavoce del cliente, rappresenta gli stakeholders;
\item \textit{Team di sviluppo (Development team)}: è il team responsabile dello sviluppo del prodotto ed è autosufficiente dal punto di vista organizzativo;
\item \textit{Scrum Master}: è il ruolo responsabile della risoluzione dei problemi che ostacolano il lavoro del team.
\end{itemize}

Particolarmente importanti nell'utilizzo di questo framework sono gli aspetti temporali che sono:
\begin{itemize}
\item \textit{Sprint}: è un'unità di tempo, fissato generalmente tra una e quattro settimane, che definisce l'avanzamento del prodotto. Il termine di ogni Sprint decreta un avanzamento incrementale del prodotto.
\item \textit{Eventi}: sono riunioni fissate in durata per ispezione e pianificazione.
\item \textit{Daily Scrum}: si tratta di una riunione giornaliera che ha lo scopo di verificare il corretto avanzamento del progetto. \lab{} effettua questo tipo di riunioni con una cadenza di 2/3 giorni.
\end{itemize}

Un altro aspetto importante che caratterizza l'uso del framework Scrum è il \textit{Product Backlog}, una lista ordinata dei requisiti da completare. L'azienda, e quindi il team, utilizza GitLab e le sue funzionalità per tenere traccia del Product Backlog, tramite l'assegnazione degli \textit{issue} contrassegnati da appropriate \textit{label}.
